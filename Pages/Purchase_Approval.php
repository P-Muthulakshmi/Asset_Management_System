...
<head>
<meta charset="UTF-8">
<title>Purchase Approval — <?php echo htmlspecialchars($prod_ref_no); ?></title>
...
<div style="margin-left: 220px; padding: 20px; text-align: center;">
  <h2 style="margin: 0; font-size: 24px; color: #002366;">Purchase Approval</h2>
  <hr style="width: 100%; border: none; border-top: 2px solid #002366; margin: 10px ;" />
</div>


<div class="content">
  <form method="POST" action="../Main/Purchase_Approval_Backend.php">
  <?php if (!empty($rows)) : ?>
  <input type="hidden" name="approval_status" value="<?php echo htmlspecialchars($rows[0]['approval_status']); ?>">
  <?php endif; ?>

  <div  class="action-bar-fixed" style="display: flex; justify-content: flex-end; margin-bottom: 40px;">
  ...
  <table id="purchaseTable">
    <thead>
      <tr>
        ...
        <th>Approval Status<span style="color:red;" >*</span></th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td>
          <?php
            ...
            $delivery_val = ($delivery_raw && $delivery_raw !== '0000-00-00') 
    ? date('d-M-Y', strtotime($delivery_raw)) 
    : '';

          ?>
          <input type="text" name="delivery_date[]" value="<?php echo htmlspecialchars($delivery_val); ?>" readonly>
        ...
        <td>
          <select name="payment_status[]" disabled>
            ...
            $current_payment = $row['payment_status'] ?? '--select--';
        ...
        <td>
          <select name="approval_status[]">
            <?php
            $apps = ['--select--','Approved','Rejected'];
            $current_approval = $row['approval_status'] ?? '--select--';

            foreach ($apps as $a) {
             $sel = ($current_approval === $a) ? 'selected' : '';
